...
class PaypalController extends Controller
{
    ...
    {
        if($this->user){
            throw new ApiException(['code' => 1, 'msg' => 'Success','data'=>['redirect'=>'/checkout/all_paypal']]);
        }else{
            $input = $request->all();
    ...
    public function index()
    {
        ...
        return $this->makeView('laravel-shop::front.checkout.all_paypal', ['res' => $res]);
    }
    ...
    public function payment(FormRequest $request)
    {
        ...
        if($this->user){
            ...
            if($input['address_id']>0){
                ...
                $request->validate($input, [
                    ...
                    'city' => 'required|alpha|between:2,128',
                    'postcode' => 'required|numeric',
                    'telephone' => 'required|numeric',
                    'country_code' => 'required|alpha|between:2,2',
                    'country_id' => 'required|numeric',
                    'zone_id' => 'required|numeric',
                ]);
                if(!$orderObj->checkPostcode($input['postcode'],$input['country_code'])){
        ...
        if($cart->hasShipping()) {
            ...
            $input['delivery_country_code'] = $userAddress['iso_code_2'];
    ...
    {
        $input = $request->all();
        if(empty($input['paypal_id'])){
            throw new ApiException(['code'=>1,'msg'=>'fail']);
        }
        (new Paypal)->returnHandle($input['paypal_id']);
    }
    ...
    public function shipping(Request $request)
    {
        ...
        if(!$shipping_id){
            throw new ApiException(['code' => 2, 'msg' => 'shipping method fail']);
        }
        $shipping_method_all = (new Shipping)->getListGuest();
        if ($shipping_method_all) {
            foreach ($shipping_method_all as $key => $val) {
                if ($key == $shipping_id && !$val['disabled']) {
                    ...
                    throw new ApiException(['code' => 0, 'msg' => 'shipping method success', 'data' => ['total_data' => $res['total_data']]]);
                }
            }
        }
    ...
    public function coupon(Request $request)
    {
        ...
        if(!empty($res['info'])){
            ...
            list($res['count'],$res['list'],$res['total_data']) = $cart->totalData();
            if(!$res['count']){
                throw new ApiException(['code'=>11,'msg'=>'no cart']);
            }
            throw new ApiException(['code'=>0,'msg'=>'success','data'=>['total_data' => $res['total_data']]]);
        }else{
