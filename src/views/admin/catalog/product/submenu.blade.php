@if(isset($res['product']) && $res['product']->id)
    ...
    <div style="display: flex;align-items: center;height: 100%;">
        <a class="badge @if($curr_path=='shop_admin/product/edit') badge-info @else badge-secondary @endif ajax_get" data-href="/shop_admin/product/edit?product_id={{$res['product']->id}}">编辑</a>
        <a class="badge @if($curr_path=='shop_admin/product/desc') badge-info @else badge-secondary @endif ajax_get" data-href="/shop_admin/product/desc?product_id={{$res['product']->id}}">描述</a>
        <a class="badge @if($curr_path=='shop_admin/product/special') badge-info @else badge-secondary @endif ajax_get" data-href="/shop_admin/product/special?product_id={{$res['product']->id}}">特价</a>
        <a class="badge @if($curr_path=='shop_admin/product/links') badge-info @else badge-secondary @endif ajax_get" data-href="/shop_admin/product/links?product_id={{$res['product']->id}}">关联</a>
