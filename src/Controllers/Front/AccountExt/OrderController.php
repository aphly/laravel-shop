...
class OrderController extends Controller
{
    ...
    {
        $res['title'] = 'My Orders';
        $res['list'] = Order::where(['uid'=>CommonUser::uid()])->where('delete_at',0)->with('orderStatus')->with('orderProduct')
            ->orderBy('created_at','desc')->Paginate(config('base.perPage'))->withQueryString();
        $res['order_cancel_24'] = $this->shop_config['order_cancel_24'];
        $res['order_cancel_24_48'] = $this->shop_config['order_cancel_24_48'];
        $res['order_cancel_48'] = $this->shop_config['order_cancel_48'];
...
//        }
    ...
    public function detail(Request $request){
        $res['info'] = Order::where(['uid'=>CommonUser::uid(),'id'=>$request->query('id',0)])->where('delete_at',0)->with('orderStatus')
    ...
    {
        $res['info'] = Order::where(['uid'=>CommonUser::uid(),'id'=>$request->query('id',0)])->where('delete_at',0)->firstOrError();
    ...
    public function cancel(Request $request)
    {
        ...
        if($res['info']->order_status_id==2 && !empty($orderHistory)){
            ...
            $comment = 'Customer voluntary cancellation: '.$cancel_fee.'% fee deducted. Final refund: '.$amount_format;
            (new Payment)->refund_api($res['info']->payment_id,$amount,$comment);
            $res['info']->addOrderHistory($res['info'], 6,['cancel_amount'=>$amount_format,'cancel_fee'=>$cancel_fee,'comment'=>$comment]);
