...
class ExtensionController extends Controller
{
    ...
    function install(Request $request){
        ...
        $extension = '\Aphly\LaravelShop\Controllers\Admin\Extension\\'.ucfirst($this->cname).'\\'.$name;
        if(class_exists($extension)) {
            (new $extension)->install();
            throw new ApiException(['code'=>0,'msg'=>'success','data'=>['redirect'=>'/shop_admin/extension/'.$this->cname.'/index']]);
        }else{
            throw new ApiException(['code'=>1,'msg'=>'fail','data'=>['redirect'=>'/shop_admin/extension/'.$this->cname.'/index']]);
        }
    }
